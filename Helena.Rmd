...
We assume that the experiment result shows that for each person in group "high", the incentive decreases the churning probability by 0.2 percent on average. And for each person in group "medium", the incentive decreases the churning probability by 0.1 percent on average
...
high_r = representative %>%
  ...
  mutate(eqpdays_churn = eqpdays_churn*1.02)
  
medium_r = representative %>%
  mutate(category = ifelse(eqpdays<=100,'low',ifelse(eqpdays<=700,'medium','high'))) %>%
  filter(category == 'medium') %>%
  mutate(eqpdays_churn = eqpdays_churn*1.01)
...
revenue <- c(rep(mean(representative_eqpdays$revenue)+25, 3*12),rep(mean(representative_eqpdays$revenue),2*12))
