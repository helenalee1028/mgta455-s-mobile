...
OR of variable 'changer' is about 1.03, which makes it less important driver in churn rate. Besides, as revenue is a mixed and comprehensive result of many influencing factor, such as pricing and actual costomer usage. We have no actionable recommendation to directly change revenue trend.
...
If we invest more in celluar tower related infrastructures, it is a one-time build up cost, the set-up cost we can afford is the difference between two CLVS. It is `r format_nr(drop_clv - base_clv, '$', dec = 2)`.

#### Profitability Evaluation of offering new equipment plans


```{r}
old_equip <- representative %>% 
  filter(eqpdays > 700)

# element-wise application
old_equip$eqpdays <- sapply(representative_equip$eqpdays, function(x) sample(c(0,x), size = 1, prob = c(0.3,0.7)))
